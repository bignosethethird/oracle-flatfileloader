------------------------------------------------------------------------------
-- $Header: vcr/views/vw_validation_position_w_fund.sql 1.3 2005/09/28 11:20:31BST apenney DEV  $
------------------------------------------------------------------------------
-- Creation script for view VCR.VW_VALIDATION_POSITION_W_FUND
--
...
-- sqlplus VCR/[password]@[instance] @vw_validation_position_w_fund.sql
------------------------------------------------------------------------------
set feedback off;
prompt Creating view VCR.VW_VALIDATION_POSITION_W_FUND

------------------------------------------------------------------------------
-- Create view
------------------------------------------------------------------------------

create or replace view VCR.VW_VALIDATION_POSITION_W_FUND (
  SOURCE_ID
, AS_OF_DATE
, BASIS
, KEY
...
, CATEGORY
, FUND_INSTRUMENT_VALUE
, FUND_TOTAL_PNL_MTD
, FUND_TOTAL_PNL_DAILY
, fund_abs_pnl_daily
) as 
SELECT  sp.source_id,
        sp.as_of_date,
       ...
       fund.instrument_value fund_instrument_value,
       fund.total_pnl_mtd    fund_total_pnl_mtd,
       fund.total_pnl_daily  fund_total_pnl_daily,
       fund.abs_pnl_daily    fund_abs_pnl_daily
       FROM source_position sp,
       (
          select /*+ INDEX(sp IX_SOURCE_POSITION_FUND) */ sp.source_id,
                sp.as_of_date, 
                sp.basis, 
                sp.fund,
                sum(sp.instrument_value) instrument_value,
                sum(abs(sp.total_pnl_daily)) abs_pnl_daily,
       ...
       WHERE sp.as_of_date = fund.as_of_date
       and   sp.source_id  = fund.source_id
       and   sp.basis      = fund.basis
       and   sp.fund       = fund.fund
;
/
